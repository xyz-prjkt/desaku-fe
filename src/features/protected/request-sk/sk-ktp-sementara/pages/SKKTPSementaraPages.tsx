...
import skKtpSementaraSchema from "@/components/general/forms/schemas/sk-ktp-sementara.schema";
import SKGeneralForm from "@/components/general/forms/SKGeneralForm";
import SKKtpSementaraForm from "@/components/general/forms/SKKtpSementaraForm";
...
import { useCreateSkKtpSementara } from "@/services/sk-ktp-sementara.service";
...
const SKKtpSementaraPages = () => {
  ...
  const onSubmit = async (data: ISkKtpSementaraCreate) => {
    await createSk(data)
      .then((res) => {
        if (res.success) {
          message.success("Berhasil mengirim permintaan");
          navigate(`/my-sk/ktp-sementara/${res.data.id}/detail`);
        }
      })
  ...
  return (
    <ContentPaper
      title="Ajukan Surat Keterangan KTP Sementara"
